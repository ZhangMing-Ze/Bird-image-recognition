...
class AudioLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.fc = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(hidden_size * 2, num_classes)  # 输出是 2 * hidden_size 到 num_classes
        )

    def forward(self, x):
        lstm_out, (h_n, c_n) = self.lstm(x)
        # lstm_out 形状: (batch_size, seq_len, 2 * hidden_size)
        # h_n 形状: (2 * num_layers, batch_size, hidden_size)
        # c_n 形状: (2 * num_layers, batch_size, hidden_size)
